...
public class PharmacyController {
    ...
    public String proceedToPharmacyDetails() {
        ...
        if (pharmacy.getOwnerMobile() == null || !pharmacy.getOwnerMobile().matches("\\d{10}")) {
        ...
        if (pharmacy.getAadhar() == null || !pharmacy.getAadhar().matches("\\d{12}")) {
            context.addMessage("form:aadharNo", new FacesMessage(FacesMessage.SEVERITY_ERROR, "Aadhar must be 12 digits.", null));
            isValid = false;
        }
    ...
    public String registerPharmacy() {
        ...
        if (pharmacy.getLicenseNo() == null || !pharmacy.getLicenseNo().matches("\\d{10}")) {
            context.addMessage("form:licenseNo", new FacesMessage(FacesMessage.SEVERITY_ERROR, "License Number must be 10 digits.", null));
            isValid = false;
        }

//        if (pharmacy.getGstNo() == null || !pharmacy.getGstNo().matches("[0-9A-Z]{15}")) {
//            context.addMessage("form:gstNo", new FacesMessage(FacesMessage.SEVERITY_ERROR, "GST Number must be 15 alphanumeric characters.", null));
//            isValid = false;
//        }
        
        String gst = pharmacy.getGstNo() != null ? pharmacy.getGstNo().toUpperCase() : "";
        pharmacy.setGstNo(gst);
        if (!gst.matches("[0-9A-Z]{15}")) {
            context.addMessage("form:gstNo", new FacesMessage(FacesMessage.SEVERITY_ERROR,
                "GST Number must be 15 uppercase alphanumeric characters.", null));
            isValid = false;
        }
        ...
        if (pharmacy.getContactNo() == null || !pharmacy.getContactNo().matches("\\d{10}")) {
